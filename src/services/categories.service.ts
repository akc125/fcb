...
export class CategoriesService {
  ...
  updateNotificationCategory(data: any, id: any) {
    return this.http.patch(
      `http://localhost:5000/notificationcategory/${id}`,
      data
    );
  }
  ...
  deleteNotificationCategoryAndActions(id: any) {
    return this.http.delete(
      `http://localhost:5000/notificationExpenseAndAction/${id}`
    );
  }
  ...
  addDebit(data: any) {
    ...
    return this.http.post('http://localhost:5000/debit', data);
  }
  addDebitAmount(data: any) {
    const id = localStorage.getItem('userId');
    console.log('addDebitAmount', data);
    return this.http.post('http://localhost:5000/debitAmount', data);
  }
